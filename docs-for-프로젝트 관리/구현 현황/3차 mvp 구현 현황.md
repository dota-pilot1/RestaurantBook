...
| `4a5ca32` | 키오스크 FSD 구조 정리 및 화면 크기별 뷰 구조 문서화 |
...
- `restaurant-book-front/src/app/about/page.tsx`
- `/about` 구현 현황 섹션

현재 `/about` 화면 기준 구현 현황은 아래와 같다.

| 구분 | 개수 | 판단 |
| --- | ---: | --- |
| 현재 구현 완료 | 16개 | 매장 주문, 주방 처리, 직원 응대, 후불 결제, 관리자 운영 관리까지 3차 MVP 운영 흐름 구성 완료 |
| 구현 예정 | 10개 | 후기, 챗봇, 예약, 외부 간편결제, SMS, QR 주문 등 후속 확장 범위 |
| 전체 항목 기준 완료율 | 61.5% | 기능 항목 수 기준 단순 비율이며, 3차 MVP 핵심 운영 흐름은 완료로 판단 |

### 0.1 현재 구현 완료

- [x] 테이블 키오스크 주문 (태블릿)
- [x] 주방 주문 접수 및 조리 상태 관리
- [x] 직원 서빙 · 호출 응답
- [x] 후불 결제 (카드 / 현금) 및 환불
- [x] 관리자 대시보드 · 매출/환불 통계 · 주문 현황
- [x] 테이블 관리 및 기본 테이블 자동 생성
- [x] 메뉴 · 카테고리 · 세트메뉴 · 품절/노출 관리
- [x] 조리 불필요 메뉴 자동 준비 완료 처리
- [x] 판매 메뉴 · 세트메뉴 엑셀 다운로드
- [x] 키오스크 메뉴 선택 체크 배지
- [x] 공지사항 · 문의 게시판 및 관리자 답변 관리
- [x] 운영 캘린더 (휴무 · 프로모션 · 단체예약 메모)
- [x] 매장 · 화면 · 내비게이션 설정 관리
- [x] 사용 가이드 · 프로필 페이지
- [x] 역할 기반 접근 제어 (고객 / 주방 / 직원 / 관리자)
- [x] 다국어 지원 (한국어 · English · 日本語 · 中文)
...
3차 MVP는 “고객 주문 → 주방 조리 → 직원 응대/결제 → 관리자 운영 관리” 흐름을 실제로 시연하고 운영 설명할 수 있는 상태다.

다만 `/about` 기준으로 남아 있는 구현 예정 항목은 결제/마케팅/고객 관리 고도화 성격이 강하다. 따라서 현재 상태는 “MVP 운영 흐름 완료, 서비스 확장 기능 진행 예정”으로 보는 것이 적절하다.
...
현재 `/about` 화면 기준으로는 완료 16개, 예정 10개다. 항목 수 기준 완료율은 61.5%지만, 남은 10개는 후기/챗봇/예약/외부 결제/SMS/QR 주문처럼 MVP 이후 확장 기능에 가깝다.

따라서 현재 상태는 아래처럼 정리한다.

- 3차 MVP 운영 흐름: 완료
- 식당 현장 발표/시연 가능 여부: 가능
- 실제 매장 파일럿 운영 가능 여부: 가능
- 결제/고객관리/마케팅 고도화: 후속 개발 필요
...
- 챗봇 자동 응답
- 예약 문의 접수 및 관리자 답변
- 카카오페이/토스페이 정식 간편결제 연동
- SMS 주문 알림 및 결제 문자 발송
- 선불 결제/키오스크 자체 결제
- 시간대별, 메뉴별, 재방문 기준 주문·매출 통계 고도화
- 테이블 QR 코드 기반 주문 지원
