...
| 커밋 | 내용 |
| --- | --- |
| `c49196e` | 직원 보드 상단 알림 카드 크기 정렬 |
| `123b542` | 키오스크 직원 호출/기본 요청 통합 다이얼로그 |
| `1a1aca3` | 직원 호출 처리 완료 문구 정리 |
| `1844ffe` | 매니저 직원 호출 미처리 메트릭 추가 |
| `2e2f7b4` | 직원 호출 운영 보드 추가 |
...
| `docs-for-필수 기능 개발-2차 mvp/고객의 직원 호출 기능 구현` | 직원 호출 도메인/API/키오스크/직원/매니저 계획 |
...
- [x] 신규 `staff_call` 도메인 추가
- [x] 직원 호출 상태 `PENDING -> ACKNOWLEDGED | CANCELED` 전이 추가
- [x] 직원 호출 생성/조회/취소/처리 API 추가
- [x] 1분 내 같은 테이블의 중복 `PENDING` 호출 서버 차단
- [x] WebSocket 토픽 `staff-calls:operations`, `customer:calls/{tableName}` 발행
- [x] 키오스크 주문 영역에 직원 호출 상태 패널과 취소 버튼 추가
- [x] 키오스크 직원 호출 다이얼로그에 기본 요청 체크박스와 직접 메시지 통합
- [x] 기본 요청 체크값과 직접 메시지를 하나의 호출 메시지로 합쳐 전송
- [x] 직원 보드 상단에 호출 알림 버튼과 미처리 건수 표시
- [x] 직원 보드 호출 다이얼로그에서 테이블/메시지/경과시간 확인 후 처리 완료 가능
- [x] 직원 보드 취소 알림과 호출 알림, 상태 메트릭 카드 크기 정렬
- [x] 매니저 대시보드에 미처리 호출 건수 메트릭 카드 추가
- [x] 호출 취소/처리 완료 시 고객 화면 상태 갱신
- [x] 호출 생성/취소/처리 완료 UX는 REST 성공 후 즉시 갱신하고 WebSocket/폴링으로 보강

관련 API:

- `POST /api/customer/staff-calls`
- `GET /api/customer/staff-calls/pending`
- `PATCH /api/customer/staff-calls/{staffCallId}/cancel`
- `GET /api/operations/staff-calls/pending`
- `PATCH /api/operations/staff-calls/{staffCallId}/acknowledge`

관련 파일:

- `restaurant-book-server/src/main/java/com/cj/restaurantbook/staff_call`
- `restaurant-book-server/src/main/java/com/cj/restaurantbook/common/websocket/AppWebSocketHandler.java`
- `restaurant-book-front/src/entities/staff-call`
- `restaurant-book-front/src/features/kiosk/KioskHome.tsx`
- `restaurant-book-front/src/features/staff-ready-orders/StaffReadyOrders.tsx`
- `restaurant-book-front/src/app/manager/dashboard/page.tsx`
...
기본 흐름은 수동 확인했고, WebSocket 연결이 불안정할 경우 폴링 보강으로 화면 갱신된다. 운영 서버 반영 전에는 백엔드 재시작 후 실시간 수신을 다시 확인하는 것이 좋다.

- [x] 키오스크 직원 호출 생성 확인
- [x] 키오스크 기본 요청 체크 + 직접 메시지 통합 전송 확인
- [x] 직원 보드 호출 알림 표시 확인
- [x] 직원 보드 호출 처리 완료 확인
- [x] 키오스크 호출 취소 확인
- [x] 직원 보드 상단 알림 카드 크기 정렬 확인
- [ ] 백엔드 재시작 후 WebSocket 즉시 반영 확인
- [ ] 1분 내 같은 테이블 중복 호출 실패 확인
- [ ] 호출 처리 완료 후 고객 화면 상태 종료 확인
- [ ] 호출 취소 후 직원 보드 알림 제거 확인
- [ ] 매니저 대시보드 미처리 호출 건수 반영 확인
...
- [ ] `StaffCallService.create()` 중복 호출 차단 테스트
- [ ] `StaffCallService.cancel()` 상태 전이 테스트
- [ ] `StaffCallService.acknowledge()` 상태 전이 테스트
...
- 매출 환불 집계 기본 흐름
- 고객 직원 호출 도메인 기본 흐름
- 키오스크 기본 요청/직원 호출 통합 UX
- 직원 보드 호출 알림 및 처리 완료 흐름
- 매니저 대시보드 미처리 호출 건수 메트릭

후속 보강으로 남길 항목:

- 환불 실패/중복 케이스 검증
- 기간 필터 경계 검증
- 직원 호출 WebSocket 즉시 반영 재확인
- 직원 호출 중복/취소/처리 완료 회귀 검증
- 자동 테스트 추가
- 운영 DB 마이그레이션 정리

추천 순서는 `직원 호출 WebSocket 재확인 -> 실패/중복 환불 검증 -> 직원 호출 중복/취소/처리 회귀 검증 -> 기간 필터 검증 -> 자동 테스트 보강`이다.
