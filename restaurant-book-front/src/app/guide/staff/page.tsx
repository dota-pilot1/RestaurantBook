...
const otherRoles = [
  { key: "customer", href: "/guide/customer", icon: ShoppingBag, accent: "text-emerald-600 dark:text-emerald-400" },
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      ...
      <nav className="mb-6 flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <span className="font-medium text-foreground">{t("staff.title")}</span>
      </nav>

      {/* Header */}
      <div className="mb-8 flex items-center gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-indigo-500 text-white shadow-sm">
          <BadgeCheck className="h-6 w-6" />
        </div>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t("staff.title")}</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">{t("staff.lead")}</p>
        </div>
      </div>
      ...
      <div className="mb-8 overflow-hidden rounded-xl border border-border bg-muted/30">
        <div className="flex aspect-video items-center justify-center">
          <div className="text-center text-muted-foreground">
            <BadgeCheck className="mx-auto mb-2 h-10 w-10 opacity-30" />
            <p className="text-xs">{t("staff.screenshotCaption")}</p>
          </div>
        </div>
      </div>
        ...
        <ol className="space-y-3">
          {steps.map((step, i) => (
            <li key={step} className="flex gap-4 rounded-lg border border-border bg-background p-4">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-sm font-bold text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300">
                {i + 1}
              </span>
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {t(`staff.steps.${step}Title`)}
                </p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t(`staff.steps.${step}Desc`)}
                </p>
              </div>
            </li>
          ))}
