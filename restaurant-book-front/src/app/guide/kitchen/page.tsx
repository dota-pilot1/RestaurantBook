...
const situations = ["s1", "s2"] as const;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      ...
      <nav className="mb-6 flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <span className="font-medium text-foreground">{t("kitchen.title")}</span>
      </nav>
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{t("kitchen.lead")}</p>
        </div>
      </div>
      ...
      <div className="mb-8 overflow-hidden rounded-xl border border-border bg-muted/30">
        <div className="flex aspect-video items-center justify-center">
          <div className="text-center text-muted-foreground">
            <Utensils className="mx-auto mb-2 h-10 w-10 opacity-30" />
            <p className="text-xs">{t("kitchen.screenshotCaption")}</p>
          </div>
        </div>
      </div>
        ...
        <ol className="space-y-3">
          {steps.map((step, i) => (
            <li key={step} className="flex gap-4 rounded-lg border border-border bg-background p-4">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-amber-100 text-sm font-bold text-amber-700 dark:bg-amber-900 dark:text-amber-300">
                {i + 1}
              </span>
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {t(`kitchen.steps.${step}Title`)}
                </p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t(`kitchen.steps.${step}Desc`)}
                </p>
              </div>
            </li>
          ))}
