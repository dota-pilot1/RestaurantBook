...
import { navigationMenuApi, type UpdateNavigationMenuBody } from "@/entities/navigation-menu/api/navigationMenuApi";
import type { NavigationMenuRecord, NavigationMenuItem } from "@/entities/navigation-menu/model/types";
import { toast, toastError } from "@/shared/lib/toast";
import { SelectInput } from "@/shared/ui/SelectInput";
import { Switch } from "@/shared/ui/Switch";
import { NavigationMenuFormDialog } from "./NavigationMenuFormDialog";

function buildTree(flat: NavigationMenuRecord[]): NavigationMenuItem[] {
...
function toUpdateBody(m: NavigationMenuRecord): UpdateNavigationMenuBody {
  return {
    parentId: m.parentId,
    label: m.label,
    labelKey: m.labelKey,
...
function collectDescendantIds(flat: NavigationMenuRecord[], parentId: number): number[] {
...
function flattenTreeForSelect(items: NavigationMenuItem[], depth = 0): Array<{ menu: NavigationMenuItem; depth: number }> {
...
function DetailPanel({
  ...
  allMenus: NavigationMenuRecord[];
  onSaved: () => void;
  onDeleted: () => void;
}) {
  const qc = useQueryClient();
  const [form, setForm] = useState<UpdateNavigationMenuBody>(toUpdateBody(menu));

  const set = <K extends keyof UpdateNavigationMenuBody>(k: K, v: UpdateNavigationMenuBody[K]) =>
    setForm((prev) => ({ ...prev, [k]: v }));

  const mutation = useMutation({
    mutationFn: () => navigationMenuApi.update(menu.id, form),
    onSuccess: () => {
      toast.success("저장되었습니다.");
      qc.invalidateQueries({ queryKey: ["navigation-menus"] });
      onSaved();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: () => navigationMenuApi.delete(menu.id),
    onSuccess: () => {
  ...
  const handleDelete = () => {
    if (!confirm(`"${menu.label}" 내비게이션 메뉴를 삭제하시겠습니까?\n하위 메뉴가 있으면 삭제할 수 없습니다.`)) return;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          <SelectInput
            ...
            options={[
              ...
              ...parentOptions.map(({ menu: option, depth }) => ({
                ...
                label: `${"　".repeat(depth)}${depth > 0 ? "└ " : ""}${option.label} (${option.code})`,
              })),
            ]}
            aria-label="부모 메뉴"
          />
        ...
        <ToggleField
          label="표시"
          description="꺼두면 헤더 내비게이션에서 숨겨집니다."
...
function TreeNode({
  ...
  onSelect: (m: NavigationMenuRecord) => void;
...
/* ── Main Component ───────────────────────────── */
export function NavigationMenuTreeTab() {
  const qc = useQueryClient();
  const [selected, setSelected] = useState<NavigationMenuRecord | null>(null);
  const [openMap, setOpenMap] = useState<Record<number, boolean>>({});
  const [localFlat, setLocalFlat] = useState<NavigationMenuRecord[] | null>(null);
  const [createOpen, setCreateOpen] = useState(false);

  const { data: serverMenus = [], isLoading } = useQuery<NavigationMenuRecord[]>({
  ...
  const flat: NavigationMenuRecord[] = localFlat ?? serverMenus;
  ...
  const saveMutation = useMutation({
    mutationFn: (items: NavigationMenuRecord[]) =>
      Promise.all(items.map((m) => navigationMenuApi.update(m.id, toUpdateBody(m)))),
    onSuccess: () => {
    ...
    <div className="grid h-[640px] gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
      ...
      <div className="flex min-w-0 flex-col overflow-hidden rounded-lg border border-border bg-background">
        <div className="flex items-center justify-between border-b border-border bg-muted/35 px-4 py-3">
          <div>
            <h2 className="text-sm font-bold">내비게이션 트리</h2>
