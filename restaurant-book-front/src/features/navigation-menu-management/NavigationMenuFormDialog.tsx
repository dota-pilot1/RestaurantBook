...
import type { NavigationMenuRecord } from "@/entities/navigation-menu/model/types";
...
type Props = {
  target: NavigationMenuRecord | "new" | null;
...
function flattenMenusForSelect(
  ...
  depth = 0
): Array<{ menu: NavigationMenuRecord; depth: number }> {
  return menus
...
export function NavigationMenuFormDialog({ target, menus, onClose }: Props) {
  const qc = useQueryClient();
  const isNew = target === "new";
  const existing = isNew ? null : (target as NavigationMenuRecord);
  ...
  const mutation = useMutation({
    mutationFn: (v: FormValues) => {
      ...
      return isNew
        ? navigationMenuApi.create({ code: v.code, ...body })
        : navigationMenuApi.update(existing!.id, body);
    },
    onSuccess: () => {
      toast.success(isNew ? "내비게이션 메뉴가 생성되었습니다." : "내비게이션 메뉴가 수정되었습니다.");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/35 px-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-lg border border-border bg-background shadow-xl">
        <div className="border-b border-border bg-muted/35 px-5 py-4">
          <p className="font-mono text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Navigation Menu</p>
          <h2 className="mt-1 text-lg font-bold tracking-tight">
            {isNew ? "내비게이션 메뉴 생성" : "내비게이션 메뉴 수정"}
          </h2>
        </div>
                ...
                <SelectInput
                  ...
                  options={[
                    ...
                    ...parentOptions.map(({ menu, depth }) => ({
                      ...
                      label: `${"　".repeat(depth)}${depth > 0 ? "└ " : ""}${menu.label} (${menu.code})`,
                    })),
                  ]}
                  aria-label="부모 내비게이션"
                />
